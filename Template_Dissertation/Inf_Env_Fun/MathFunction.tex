...
% Rotation matrix:
%   \Rot{W}{B}    -> {}^{W}_{B}\mathbf{R}
%   \Rot[i]{W}{B} -> {}^{W}_{B}\mathbf{R}_{i}
\newcommand{\Rot}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}_{#3}]{\mathbf{R}}{}%
    \else
        \tensor*[^{#2}_{#3}]{\mathbf{R}}{_{#1}}%
    \fi
    }%
}

% Estimated / predicted rotation matrix:
%   \EstRot{W}{B}    -> {}^{W}_{B}\hat{\mathbf{R}}
%   \EstRot[i]{W}{B} -> {}^{W}_{B}\hat{\mathbf{R}}_{i}
\newcommand{\EstRot}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}_{#3}]{\hat{\mathbf{R}}}{}%
    \else
        \tensor*[^{#2}_{#3}]{\hat{\mathbf{R}}}{_{#1}}%
    \fi
    }%
}

% Homogeneous transformation matrix:
%   \Tf{W}{B}    -> {}^{W}_{B}\mathbf{T}
%   \Tf[i]{W}{B} -> {}^{W}_{B}\mathbf{T}_{i}
\newcommand{\Tf}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}_{#3}]{\mathbf{T}}{}%
    \else
        \tensor*[^{#2}_{#3}]{\mathbf{T}}{_{#1}}%
    \fi
    }%
}

% Estimated / predicted homogeneous transformation matrix:
%   \EstTf{W}{B}    -> {}^{W}_{B}\hat{\mathbf{T}}
%   \EstTf[i]{W}{B} -> {}^{W}_{B}\hat{\mathbf{T}}_{i}
\newcommand{\EstTf}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}_{#3}]{\hat{\mathbf{T}}}{}%
    \else
        \tensor*[^{#2}_{#3}]{\hat{\mathbf{T}}}{_{#1}}%
    \fi
    }%
}

% Translation vector:
%   \tvec{W}{B}    -> {}^{W}\mathbf{t}^{W/B}
%   \tvec[i]{W}{B} -> {}^{W}\mathbf{t}_{i}^{W/B}
\newcommand{\tvec}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}]{\mathbf{t}}{^{#2/#3}}%
    \else
        \tensor*[^{#2}]{\mathbf{t}}{_{#1}^{#2/#3}}%
    \fi
    }%
}
...
\newcommand{\RelMeas}[3][]{%
    \ensuremath{%
    \if\relax\detokenize{#1}\relax
        \tensor*[^{#2}_{#3}]{\mathbf{Z}}{}%
    \else
        \tensor*[^{#2}_{#3}]{\mathbf{Z}}{_{#1}}%
    \fi
    }%
}
